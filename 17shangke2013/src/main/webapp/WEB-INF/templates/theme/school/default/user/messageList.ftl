...
<dl class="list-item message-item" id="messageInfo_dl_${m.message_id}" action-type="message-item" messageid="${m.message_id}">
	...
	<dd class="content">
		<p>			
			...
			系统消息
            <#else><a href="/u/${m.sender}.html" target="_blank" usercard="${m.sender}" >${m.sender}</a>
			</#if>：
...
<dl class="list-item message-item"
	...
	<dd class="content">
		<p>			
            ...
            <span class="message-status${messageType}">${bbcodeHelper.doFilter(m.message,"1").getReplaceContent()}</span>		
		</p>
		<p class="data-info">
			<span class="link-group float-right">
...
<div class="private_shresults">
  ...
  <#else>
	  ...
	  <#if m.sender?c!=USER_ID>
	    <dl class="private_SRLl clearfix" id="messageInfo_dl_${dlIndex}"><!--发送者-->
	  <#else>
		<dl class="private_SRLr clearfix" id="messageInfo_dl_${dlIndex}"><!--接收者-->
	  </#if>
	    <dt class="face">
  ...
  <#if m_has_next&&result.result[m_index+1].sender!=m.sender><!--如果下一条记录不是本人-->
  	 	<div class="arrow"></div>
	    </dd>
	  </dl>
