                ...
                <#if (course.status!=0)>
                    ...
                    <#if course.status=-1> 
                     ...
                     <#elseif course.status==1>
                         <#--课程取消-->  
                         <#if course.type==1> 
                            <a href="javascript:;"  node-type="course-cancle">取消</a> |
                        </#if>  
                    </#if> 
                    <#--未结束的课程才允许编辑-->
                    <#if (course.status<8) ||(course.type=2||course.type==4)>
                        <a href="${SITE_DOMAIN}/course/manager/${course.courseId}/edit.html" target="_blank">编辑</a> |
                    <#else>  
                         <#--对已结课课程进行关闭-->
                         <#if course.status!=10 >
                         <a href="javascript:;"  node-type="course-close" >关闭</a> | 
                         </#if>
                    </#if>
                </#if>
			</#if>
