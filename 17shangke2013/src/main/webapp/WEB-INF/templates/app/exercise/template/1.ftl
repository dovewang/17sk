...
<div style="width:720px;">
	...
	<form id="exerciseForm" action="/q/update.html" onsubmit="return false;">
		...
		<input type="hidden" id="exercise_type" name="type" value="1"/>
		<input type="hidden" id="exercise_picks" name="picks" value="1"/>
		<input type="hidden" id="exercise_answer" name="answer" value="1"/>
		<input type="hidden" id="questionId" name="questionId" value="${result.questionId}"/>
		<dl class="form">
			<dd>
				<label>请输入问题内容：</label>
			</dd>
			...
			<dd>
				...
				</span>&nbsp;&nbsp;&nbsp;&nbsp;<span><a href="javascript:void(0)" node-type="kownleadge-select">知识点设置</a><span id="setKnowledgeDiv" style="padding-left:20px;"></span></span>
			</dd>
			<dd id="exercise_answerArea">
				答案：
				<#assign pick=result.picks?split(",")/>
				<#list pick as p> <span>${p}.
					<input name="option" type="radio" value="${p}" <#if result.answer==p>checked="checked"</#if> />
				</span>
	...
	<form id="exerciseForm" action="/q/save.html" onsubmit="return false;">
		...
		<dl class="form">
			...
			<dd id="exercise_answerArea">
				...
				<span>C.
					<input name="option" type="radio" value="C" />
				</span>
				<span>D.
					<input name="option" type="radio" value="D" />
				</span>
