...
<div style="width:700px;">
	...
	<form id="exerciseForm" action="/q/update.html" onsubmit="return false;">
		...
		<input type="hidden" id="exercise_type" name="type" value="3"/>
		...
		<dl class="form">
			...
			<dd id="exercise_answerArea">
				答案：<span>对.
					<input name="option" type="radio" value="1" <#if result.answer=="1">checked="checked"</#if> />
				</span>
				<span>错.
					<input name="option" type="radio" value="0" <#if result.answer=="0">checked="checked"</#if>/>
				</span>
			</dd>
			<dd>
				答案评析：
